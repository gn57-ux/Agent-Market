...
import { RESULT_APPROVED_EVENT_ABI } from "../chain/result-approved-event.js";
import { DELIVERY_TIMEOUT_CLAIMED_EVENT_ABI } from "../chain/delivery-timeout-claimed-event.js";
import { REVIEW_TIMEOUT_FINALIZED_EVENT_ABI } from "../chain/review-timeout-finalized-event.js";
  ...
  async function insertTaskWithStatus(status: "ACCEPTED" | "SUBMITTED"): Promise<{
    ...
    const { rows } = await pool.query<{ id: string }>(
      `INSERT INTO tasks
       ...
       VALUES ($1, 'writing', 'Task', 'desc', $2, $3, '2099-01-01T00:00:00Z', $4, $5, $6, now())
       RETURNING id`,
      [
        requester.address.toLowerCase(),
        TASK_BUDGET,
