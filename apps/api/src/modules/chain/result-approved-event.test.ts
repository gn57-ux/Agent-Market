...
import {
  ...
  findResultApprovedLog,
  RESULT_APPROVED_EVENT_ABI,
...
function buildResultApprovedLog(overrides: {
  address?: string;
  taskId?: `0x${string}`;
  agent?: `0x${string}`;
  budget?: bigint;
...
describe("decodeResultApprovedLog", () => {
  it("decodes a real, ABI-encoded ResultApproved log", () => {
    const log = buildResultApprovedLog({});
  ...
  it("returns null (not a throw) for a log whose topics don't match the ResultApproved signature", () => {
    const log = buildErc20TransferLog();
    expect(decodeResultApprovedLog(log)).toBeNull();
  });
...
describe("findResultApprovedLog", () => {
  it("finds the ResultApproved log emitted by trustedAddress, skipping a Transfer log from another contract", () => {
    const transferLog = buildErc20TransferLog({ address: TOKEN_ADDRESS, logIndex: 0 });
    const approvedLog = buildResultApprovedLog({ address: TASK_ESCROW_ADDRESS, logIndex: 1 });

    const decoded = findResultApprovedLog([transferLog, approvedLog], TASK_ESCROW_ADDRESS);
  ...
  it("is case-insensitive when matching the trusted address", () => {
    const approvedLog = buildResultApprovedLog({
      address: TASK_ESCROW_ADDRESS.toUpperCase().replace("0X", "0x"),
    });
    const decoded = findResultApprovedLog([approvedLog], TASK_ESCROW_ADDRESS.toLowerCase());
    expect(decoded).not.toBeNull();
  });

  it("skips a ResultApproved-shaped log emitted from an untrusted address", () => {
    const otherAddress = "0x999999999999999999999999999999999999999b";
    const wrongAddressLog = buildResultApprovedLog({ address: otherAddress });

    const decoded = findResultApprovedLog([wrongAddressLog], TASK_ESCROW_ADDRESS);
    expect(decoded).toBeNull();
  });

  it("returns null when no log matches", () => {
    const transferLog = buildErc20TransferLog({ address: TOKEN_ADDRESS });
    expect(findResultApprovedLog([transferLog], TASK_ESCROW_ADDRESS)).toBeNull();
  });
});
